...
//Test making a node tree, removing some nodes, then rewrite a file to that location in memory
int main()
{
  ...
  while (num_read < fs_root->num_children) {
    ...
    new_table_entry = NULL;
    num_read = 0;
    idx = 0;
    
    

    f_remove(filenode, file_fd2);
    char data5[100];
    char data6[100];
    const char* text3 = "appending this message.";
    memset(data5, 0 , sizeof(data));
    memset(data6, 0 , sizeof(data));
    size_t text3len = strlen(text3);
    strncpy(data5, text3, text3len);

    vfd_t file_fd3 = f_open(dirnode, "B.txt", flags);
    vnode_t* filenode3 = f_find(dirnode, "B.txt");

    size_t size3 = f_write(filenode3, data5, text3len, 1, file_fd3);
    printf("here now size %ld\n", size3);


  
    f_seek(filenode3, file_fd2, 0, SEEK_SET);
    f_read(filenode3, data6, text3len, 1, file_fd3);
    f_close(filenode3, file_fd3);
    printf("retrieved data: %s\n", data6);
    f_close(filenode3, file_fd3);

    while (num_read < dirnode->num_children) {
      new_table_entry = f_readdir(dirnode, idx);
      printf("idx %d\n", idx);
      if (new_table_entry != NULL && new_table_entry->valid == 1) {
        //to make sure that size is the same, indicate that you are erasing/overwriting one vdir
        //this is where we want to write
     ...
     //TODO: make sure vnode tree is updated
